...
function [images, labels] = images_and_labels_bp(new_dataset, window_row, window_colm, window_stride, mat_names, k,std_dev)
    ...
    for mat_idx = 1: size(new_dataset, 1)
        ...
        img{mat_idx,1} = cat(4, img{mat_idx,1}{:});
        img{mat_idx,1} = permute(img{mat_idx,1}, [4 1 2 3]);
        disp(mat_idx);

        raw_img{mat_idx,1} = vertcat(raw_images_data{mat_idx,:});

        raw_img{mat_idx,1} = cat(4, raw_img{mat_idx,1}{:});

        raw_img{mat_idx,1} = permute(raw_img{mat_idx,1}, [4 1 2 3]);
        disp(mat_idx);
    ...
    raw_images = permute(raw_images, [2 3 4 1]);
    disp(size(images));
    disp(size(raw_images));
    %[sigma_R_Sqr, d_new, G] = sigma_R_Square(images);
    images = knn_bp(images, raw_images, k);
    disp(size(images));
