...
function knn_images = knn_bp(images, raw_images, k)
    ...
    knn_image = cell(size(images, 4), 1);
    for n = 1: size(images, 4)

        b_image = images(:,:,:,n) - repmat(images(centreR, centreC, :, n), size(images,1), size(images,2));
         % b_image = reshape(b_image, [ 5, 5, 2]); changed for before_pca
        b_image = reshape(b_image, [ 5, 5, 1]);
